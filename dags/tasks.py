...
def get_postcode_dict(uk_postcode_df):
    postcode_dict = dict(zip(uk_postcode_df['postcode'],
                         zip(uk_postcode_df['latitude'],
                         uk_postcode_df['longitude'])))
...
def get_10years_houseprice(uk_houseprice_df):
    ...
    house_price_sub = house_price_10years.drop(['name_7',
                                                'name_8',
                                                'name_10',
                                                'name_15'], axis=1)
    specfic_column = ['unique_id', 'price', 'date_of_transfer',
                      'postcode', 'property_type', 'old_or_new',
                      'duration', 'address', 'city', 'district',
                      'county', 'ppd_category', 'year']
...
def get_property_type_csv(postcode_houseprices_path):
    ...
    for p_name, p_type in property_type_list:
        ...
        property_csv = postcode_houseprices_df[
                        postcode_houseprices_df['property_type']==p_type]
...
# sql alchemy to copy df to created table
    connection_string = f"postgresql://{USERNAME}:{PASSWORD}@{HOST_NAME}/{DATABASE_NAME}"
...
def make_database_connection():
    ...
    CSVFILES = [csv_file for csv_file in os.listdir(DATASET_DIR)
                if csv_file.endswith('.csv')
                and not (csv_file.endswith('10years.csv')
                or csv_file.endswith('prices.csv'))]

    for csv_file in CSVFILES:
        tablename = csv_file.split('.')[0]
        csv_filepath = f"{DATASET_DIR}/{csv_file}"
        csv_df = pd.read_csv(csv_filepath)
        df_columnlist = [str(col) for col in list(csv_df.columns)
